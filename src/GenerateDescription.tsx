...
const conjugateHave = (pronouns: PronounsObj): string => {
  return pronouns.subjective === "he" || pronouns.subjective === "she"
    ? `${pronouns.subjective} has`
    : `${pronouns.subjective} have`;
};
...
const getSecondSentence = (pronouns: PronounsObj): string => {
  ...
  sentences[1] = `${capitalizeFirstChar(pronouns.possessive)} demeanor was ${
    getRandom(demeanors)[0]
  }, ${pronouns.possessive} companion a connoisseur of ${
    getRandom(connoisseurOf)[0]
  }, ${
    pronouns.possessive
  } name an hommage to a distant relative who innovated designs for ${
    getRandom(designsFor)[0]
  }.`;
  let occ = getRandom(secondaryOccupations, 2);
  let trait = getRandom(personalityTraits)[0];
  sentences[2] = `Born to ${getArticle(occ[0])} and ${getArticle(trait)} ${
    occ[1]
  }, ${pronouns.subjective} excelled from an early age in the subtle art of ${
    getRandom(subtleArts)[0]
  }.`;
  sentences[3] = `Convicted for ${getRandom(convictedFor)[0]} at a young age, ${
    pronouns.subjective
  } subsequently turned to ${getRandom(connoisseurOf)[0]} as new target for ${
    pronouns.possessive
  } seemingly inexhaustible energies.`;
  sentences[4] = `${capitalizeFirstChar(
    pronouns.subjective
  )} ran with a crew called ${getRandom(crews)[0]} who were known for ${
    getRandom(convictedFor)[0]
  } and controlled territory ${getRandom(territories)[0]}.`;
  sentences[5] = `Like most violent young people - and not a few stuffy old ones - ${
    pronouns.subjective
  } found the idea of defeat hateful, and so devoted ${
    pronouns.objective
  }self to becoming a world-renowned master practitioner in ${
    pronouns.possessive
  } chosen field of ${getRandom(subtleArts)[0]}.`;
  sentences[6] = `${capitalizeFirstChar(pronouns.subjective)} ${conjugateisPast(
    pronouns
  )} a Catholic; had been to a convent school near ${
    pronouns.possessive
  } home.  ${capitalizeFirstChar(pronouns.subjective)} talked overmuch about ${
    pronouns.possessive
  } religion, and had indeed for a time considered the Son of God as a young person will consider any eligible bachelor.`;
  sentences[7] = `${capitalizeFirstChar(
    pronouns.possessive
  )} life's progress had been inevitably east; having somehow escaped the hothouse of ${
    pronouns.possessive
  } fellow countrymen ${
    pronouns.subjective
  } flew to the other extreme and developed an obsession with ancestral roots.  Land of God.  Land of suffering, also.  Scenes of specific persecution upset ${
    pronouns.objective
  }.`;
  ...
  sentences[9] = `${capitalizeFirstChar(
    pronouns.subjective
  )} spoke 33 languages including English with a strong Oxonian blither to it.`;
  ...
  sentences[11] = `${capitalizeFirstChar(
    pronouns.subjective
  )} studied composition in Paris and had gained a certain notoriety among the avant-garde in French music.  Opinion in the city was violently divided: once the composer had been loudly insulted in the street by one of the most venerable of the Post-Romantics.`;
  ...
  sentences[13] = `Trained as a plastic surgeon ${
    pronouns.subjective
  } referred to ${
    pronouns.possessive
  } profession as the art of Tagliacozzi.  ${capitalizeFirstChar(
    pronouns.possessive
  )} own methods, while not as primitive as those of the sixteenth-century Italian, were marked by a certain sentimental inertia, so that ${
    pronouns.subjective
  } was never quite up to date.`;
  ...
  sentences[15] = `A recent escapee of Devil's Island, ${
    pronouns.subjective
  } ${conjugateisPast(
    pronouns
  )} now en route to Vassar under the alias of Maynard Basilisk to teach beekeeping.`;
  sentences[16] = `${capitalizeFirstChar(
    pronouns.subjective
  )} could be found on weekends attending meetings with a small group of Tyrosemiophiles (collectors of labels on French cheese boxes) whom ${
    pronouns.subjective
  } met at an annual convention on this oft-overlooked but nevertheless thriving hobby.`;
...
export const GenerateDescription = (
  ...
  const descType: keyof Descriptor = (
    Object.keys(descriptors) as Array<keyof Descriptor>
  ...
  const descString = retired
    ? `a retired ${nationality} ${occupation}`
    : `${getArticle(nationality)} ${occupation}`;
